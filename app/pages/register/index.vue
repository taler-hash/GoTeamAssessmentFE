    ...
    <Card class="w-full max-w-md mx-4 relative z-10 shadow-2xl">
      ...
      <template #title>
        <div class="text-center">
          ...
          <p class="text-gray-600 text-sm">Register to continue using this app</p>
        </div>
      </template>
...
const onSubmit = async (values: User) => {
  ...
  try {
    ...
    await $fetch(`${config.public.base_api_url}auth/register`, {
      method: 'POST',
      body: values,
    });
    ...
    toast.add({ severity: 'success', summary: 'Success', detail: 'Successfully registered', life: 3000 });
    toast.add({ severity: 'success', summary: 'Redirecting to dashboard', life: 3000 });
    ...
    toast.add({ 
      ...
      detail: (error as any)?.data?.message ?? (error as any)?.message ?? 'Registration failed', 
      life: 3000,
    });
